...
namespace dsemi {
	namespace ecs {
		entity_array::entity_array(size_t num_components)
			: _next_id(0),
		...
		size_t entity_array::create_entity()
		{
		...
		void entity_array::set_component(size_t entity_id, size_t component_id, bool value)
		{
		...
		bool entity_array::get_component(size_t entity_id, size_t component_id)
		{
			if (entity_id * _entity_size > _entity_bytes.capacity())
				return false;
		...
		component_flag entity_array::get_components(size_t entity_id)
		{
			component_flag out(_entity_size * 64u);
		...
		component_flag entity_array::operator[](size_t entity_id)
		{
			if (entity_id * _entity_size > _entity_bytes.capacity())
				return component_flag(0);
